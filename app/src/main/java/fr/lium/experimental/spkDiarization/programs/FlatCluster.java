...
/**
 * The Class FlatCluster.
 */
public class FlatCluster {

	/** The Constant logger. */
	private final static Logger logger = Logger.getLogger(FlatCluster.class.getName());
	...
	public static String putSpeakerName(SCTProbabilities probabilities, String speakerName, TurnSet turnSet, int index) {
		...
		if (nameAndGenderMap.get(normalizedSpeakerName) != null) {
						...
						if (SpeakerNameUtils.checkSpeakerName(person, isCloseListCheck, nameAndGenderMap, firstNameAndGenderMap) == true) {
							for (Cluster cluster : audioClusterSet.getClusterVectorRepresentation()) {
								for (Segment segment : cluster) {
									...
									speakerName.addScoreCluster(rate);
								}
							}
						} else {
				...
				for (int index = 0; index < linkSet.size(); index++) {
					...
					if (link.haveEntity(EntitySet.TypePersonne) == true) {
						...
						if (SpeakerNameUtils.checkSpeakerName(speakerName, isCloseListCheck, nameAndGenderMap, firstNameAndGenderMap) == true) {
							String normalizedName = putSpeakerName(probabilities, speakerName, turns, i);
		...
		try {
			...
			if (parameter.show.isEmpty() == false) {
				...
				for (String name : videoClusterSet) {
					videoClusterSet.getCluster(name).clearSpeakerNameSet();
				}
				putWriting(videoClusterSet, parameter.getParameterNamedSpeaker().getThresholdWriting());
				computeBeliefFunctions(videoClusterSet);
				videoClusterSet = decideMaximum(videoClusterSet, scoreKeys[4], nameKeys[4]);
				...
				computeBeliefFunctions(videoClusterSet);
				videoClusterSet = decideMaximum(videoClusterSet, scoreKeys[5], nameKeys[5]);
